...
export default function Home() {
  ...
  const load = async () => {
    ...
    const { data: postsData, error } = await supabase
      .from('posts')
      .select('id, body, image_url, user_id, created_at, is_hidden, likes(user_id)')
      .eq('is_hidden', false)
  ...
  const post = async () => {
    ...
    try {
      ...
      const { error } = await supabase.rpc('create_post_with_image', {
        post_body: body || '', image_url: imageUrl,
      })
      if (error) alert('投稿失敗: ' + error.message)
      else { setBody(''); setFile(null); await load() }
  ...
  const editPost = async (postId: number, currentBody: string) => {
    ...
    const { error } = await supabase.rpc('edit_my_post', { target_post_id: postId, new_body: newBody })
    if (error) alert(error.message); else load()
  }

  const reportPost = async (postId: number) => {
    const reason = prompt('通報理由を入力してください (3-500文字):')
    if (!reason || reason.length < 3) return
    const { error } = await supabase.rpc('report_post', { p_post_id: postId, p_reason: reason })
    if (error) alert(error.message)
    else { alert('通報を受け付けました'); load() }
  }

  const blockUser = async (userId: string) => {
    if (!confirm('このユーザーをブロックしますか?投稿が見えなくなります。')) return
    const { error } = await supabase.rpc('toggle_block', { target_user_id: userId })
    if (error) alert(error.message); else { alert('ブロックしました'); load() }
  }
    ...
    <div className="max-w-xl mx-auto p-4">
        ...
        <div className="text-sm flex items-center gap-3 flex-wrap">
          {user ? (
            <>
              ...
              {isAdmin && <a href="/admin" className="text-red-500 underline">管理</a>}
      ...
      {user && (
        <div className="mb-4 border rounded-lg p-3">
          <textarea className="border w-full p-2 rounded" value={body}
            onChange={e => setBody(e.target.value)} maxLength={500}
            placeholder="いまどうしてる?" rows={3} />
          {file && (
        ...
        {visiblePosts.map(p => (
          <li key={p.id} className="border-b py-3">
            <div className="text-sm text-gray-500 mb-1 flex justify-between items-center">
              <span>@{p.profile?.handle || 'unknown'} ({p.profile?.display_name || '名無し'})</span>
            </div>
            ...
            <div className="flex gap-3 text-sm text-gray-600 mt-2 flex-wrap">
              ...
              {user && p.user_id === user.id && (
                <>
                  <button onClick={() => editPost(p.id, p.body || '')}
                    className="text-blue-500 hover:underline">編集</button>
                  <button onClick={() => deletePost(p.id)}
                    className="text-red-500 hover:underline">削除</button>
                </>
              )}
              {user && p.user_id !== user.id && (
                <>
                  <button onClick={() => reportPost(p.id)}
                    className="text-orange-500 hover:underline">通報</button>
                  <button onClick={() => blockUser(p.user_id)}
                    className="text-gray-500 hover:underline">ブロック</button>
                </>
              )}
            </div>
          </li>
